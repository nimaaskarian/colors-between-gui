...
static inline Color colorBlack()
{
  Color output;
  output.r = 0;
  output.g = 0;
...
static inline double relativeLuminanceStdColor(StdColor srgb)
{
...
static inline double relativeLuminance(Color color)
{
  ...
  return relativeLuminanceStdColor(srgb);
}
...
static inline Color colorFromQColor(QColor qcolor)
{
  Color color;
  color.r = qcolor.red();
...
static inline QString stdColorQString (StdColor srgb)
{
  QString output("#%1%2%3");
  Color color = StdColorToColor(srgb);
  // swprintf(output.data_ptr().ptr,"#%02x%02x%02x\n", color.r, color.g, color.b);
  return output.arg(color.r,2,16, QChar('0'))
